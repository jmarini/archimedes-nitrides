...
// =============================
 if(g_config->qep_model==QEP_CALIBRATED_BOHM || g_config->qep_model==QEP_BOHM){
  ...
  for(i=3;i<=nx-1;i++)
    for(j=3;j<=ny-1;j++){
     if(n_x[i][j]>0.0){
      tmp=((MSTAR[g_mesh->info[i+1][j].material][1]-MSTAR[g_mesh->info[i][j].material][1])/(MSTAR[g_mesh->info[i+1][j].material][1]*MSTAR[g_mesh->info[i][j].material][1]*M))
                *E*pow(n_x[i][j],D)*(n_x[i+1][j]-n_x[i][j])*odx2
               +((MSTAR[g_mesh->info[i][j+1].material][1]-MSTAR[g_mesh->info[i][j].material][1])/(MSTAR[g_mesh->info[i][j+1].material][1]*MSTAR[g_mesh->info[i][j].material][1]*M))
               ...
               /(MSTAR[g_mesh->info[i][j].material][1]*M);
...
//  real tmp;
  ...
  for(i=2;i<=nx;i++)
   for(j=2;j<=ny;j++){
    // electron wavelenght
    alpha2=HBAR*HBAR/(8.*M*MSTAR[g_mesh->info[i][j].material][1]*KB*g_config->lattice_temp);
