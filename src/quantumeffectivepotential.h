...
int quantum_effective_potential( ) {
    int i,j,k;
    double dx = g_mesh->dx,
           dy = g_mesh->dy;
    int nx = g_mesh->nx,
        ny = g_mesh->ny;
    double small=1.e-24;
    double n_x[NXM+1][NYM+1];
    double n_y[NXM+1][NYM+1];
    double u[NXM+1][NYM+1];
    double p[5];
    double tmp;
    double Qeff[NXM+1][NYM+1];
    double odx2=1./(dx*dx);
    double ody2=1./(dy*dy);
    double C,D,E;
    double A1=5.;
    ...
    if(g_config->qep_model == QEP_FULL) {
        ...
        for(i=2;i<=nx;i++) {
            for(j=2;j<=ny;j++){
                ...
                Qeff[i][j]=alpha2*((g_mesh->nodes[i+1][j].potential-2.*g_mesh->nodes[i][j].potential+g_mesh->nodes[i-1][j].potential)*odx2
                          +(g_mesh->nodes[i][j+1].potential-2.*g_mesh->nodes[i][j].potential+g_mesh->nodes[i][j-1].potential)*ody2);
            }
        }
